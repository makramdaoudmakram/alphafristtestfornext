      ...
      {details.map((row) => {
        ...
        return (
          <div key={row.clientRowId} className={cn("rounded-lg border p-3", DETAIL_GRID_CLASS)}>
            ...
            <div className="space-y-1">
              <p className="text-muted-foreground text-xs md:hidden">Stock</p>
              <div className="flex flex-col gap-0.5 text-xs leading-tight">
                ...
                <span className="text-muted-foreground tabular-nums">
                  Pending transfer:{" "}
                  {Number.isFinite(row.itmTransferQty) ? row.itmTransferQty : 0}
                </span>
                <span className="tabular-nums">
                  Available:{" "}
                  <span className="font-medium">
                    {Number.isFinite(row.itmAvailableQty)
                      ? row.itmAvailableQty
                      : Number.isFinite(row.itmStockQty)
                        ? row.itmStockQty
                        : 0}
                  </span>
                </span>
