...
import {
  applyPharmPurchaseDetailPatch,
  computePharmPurchaseHeaderTotals,
...
import {
  createPharmPurchaseService,
  PharmPurchaseRepositoryError,
} from "@/services/pharm-purchase.service";
...
export function usePharmPurchase(token: string | undefined) {
  ...
  const detailsWithTotals = useMemo(
    () => details.map((row) => applyPharmPurchaseDetailPatch(row, {})),
    [details]
  );

  const computedTotals = useMemo(
    () => computePharmPurchaseHeaderTotals(detailsWithTotals),
    [detailsWithTotals]
  );
  ...
  const service = useMemo(
    () => (token ? createPharmPurchaseService(token) : null),
    [token]
  );

  const openInsertedStockBarcodePrint = useCallback(
    async (_recordId: number | null | undefined) => {
      // Pharmacy purchase does not expose barcode print in this version.
    },
    []
  );
  ...
  const loadRecord = useCallback(
    async (
      ...
      try {
        ...
        setMode("view");
        toast.success(`Loaded pharmacy purchase #${doc.header.pthId ?? id}`);
        return catalogMap;
      } catch (error) {
        toast.error(
          error instanceof Error ? error.message : "Failed to load pharmacy purchase"
        );
        return undefined;
      } finally {
        setLoading(false);
  ...
  const handleSave = useCallback(
    async (
      ...
      if (removedCount > 0 && !allowEmptyDetails) {
        setDetails(detailsForSave.map((row) => applyPharmPurchaseDetailPatch(row, {})));
        ...
        const label =
          saved.header.pthId != null
            ...
            : saved.header.id != null
              ? `ID ${saved.header.id}`
              : "";
        toast.success(label ? `Pharmacy purchase saved ${label}` : "Pharmacy purchase saved");
        if (removedCount > 0) {
  ...
  const handlePrintBarcode = useCallback(async () => {
    toast.message("Barcode printing is not available on Pharmacy Purchase.");
  }, []);
  ...
  const runSearch = useCallback(
    async (filters: PurchaseSearchFilters) => {
      ...
      try {
        ...
        if (
          error instanceof PharmPurchaseRepositoryError &&
          error.status === 404
        ) {
          return [];
        }
  ...
  const updateDetailRow = useCallback(
    (index: number, patch: PurchaseDetailPatch) => {
      setDetails((rows) =>
        rows.map((row, i) =>
          i === index ? applyPharmPurchaseDetailPatch(row, patch) : row
        )
      );
    },
    []
...
export type UsePharmPurchaseReturn = ReturnType<typeof usePharmPurchase>;
