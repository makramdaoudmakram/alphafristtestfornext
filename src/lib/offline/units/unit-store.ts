...
export async function removeCachedUnit(uCode: number): Promise<void> {

  const db = getOfflineDb();

  await db.units.delete(uCode);

}



export async function clearUnitPendingSync(uCode: number): Promise<void> {

  const db = getOfflineDb();

  const row = await db.units.get(uCode);
