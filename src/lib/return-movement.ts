...
import type { ReturnHeader } from "@/types/return";
...
/** Resolve Movment lookup row for a saved return header (MovId = MovChiledId in DB). */
export async function resolveMovementForReturnHeader(
  token: string,
  parentId: number,
  header: Pick<
    ReturnHeader,
